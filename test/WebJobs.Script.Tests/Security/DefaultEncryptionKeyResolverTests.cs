    ...
    public class DefaultEncryptionKeyResolverTests
    {
        ...
        public void ResolveKey_WithDefaultKeyInAzure_ResolvesEnvironmentKey()
        {
            ...
            using (var testVariables = new TestScopedEnvironmentVariable(new Dictionary<string, string>
            {
                ...
                { EnvironmentSettingNames.AzureWebsiteEncryptionKey, testKey },
            }))
            {
    ...
    <system.web>
      <machineKey validationKey=""NONE"" decryptionKey=""{testKey}"" decryption=""AES"" />
        ...
        public void ResolveKey_WithKeyId_ResolvesEnvinronmentKey()
        {
            ...
            using (var testVariables = new TestScopedEnvironmentVariable(new Dictionary<string, string>
            {
                { EnvironmentSettingNames.AzureWebJobsDefaultEncryptionKeyId, testKeyId },
                { testKeyId, testKey },
            }))
            {
                var resolver = new DefaultEncryptionKeyResolver();
        ...
        [InlineData(null)]
        [InlineData("")]
        [InlineData(DefaultEncryptionKeyResolver.DefaultEncryptionKeyId)]
        [InlineData(EnvironmentSettingNames.AzureWebsiteEncryptionKey)]
        public void ResolveKey_WithDefaultKeyLocal_ResolvesEnvironmentKey(string keyId)
        {
            string testKey = "0F75CA46E7EBDD39E4CA6B074D1F9A5972B849A55F91A248";
            CryptographicKey expectedKey = CryptographicKey.FromHexString(EnvironmentSettingNames.AzureWebJobsLocalEncryptionKey, testKey);

            using (var testVariables = new TestScopedEnvironmentVariable(EnvironmentSettingNames.AzureWebJobsLocalEncryptionKey, testKey))
            {
                var resolver = new DefaultEncryptionKeyResolver();
